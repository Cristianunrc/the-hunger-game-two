...
class GameController:  
      def get_game(self, data):
        ...
        game.new_game(15, 15)
        try:
            game.set_parameters(data['life'], data['force'], data['alliance'],
                                data['cant_tributes'], data['cowardice'])
      ...
      def pause_game(self, curr_game):
          lives = curr_game.tributes_lives()
          return lives
      
      # Return one list of health of each distric's 
      def life_of_each_district(self, curr_game):
          district_life = curr_game.district_lifes()
